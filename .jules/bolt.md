...
## 2025-06-27 - Converting DataFrame to list of dicts for iteration
**Learning:** Iterating over large Pandas DataFrames using `df.iterrows()` is extremely slow because it creates a Series object for each row.
**Action:** When row-wise iteration is needed, converting the DataFrame to a list of dicts first using `df.to_dict('records')` is much faster.
