...
## 2024-05-20 - df.iterrows() is an anti-pattern
**Learning:** `df.iterrows()` is extremely slow for large dataframes because it instantiates a Pandas Series for every row.
**Action:** Replace it with `df.to_dict('records')` to convert the entire dataframe into a list of dictionaries, which iterates much faster in pure Python loops.
