...
## 2025-02-12 - [DataFrame Iteration Optimization]
**Learning:** In Pandas, iterating over large DataFrames using `df.iterrows()` is an anti-pattern that creates significant bottlenecks because it generates a new Series object for every row.
**Action:** Always convert the DataFrame to a list of native Python dictionaries using `df.to_dict('records')` first, which performs a highly optimized conversion in C. Downstream usages of the row object must be updated to handle native dicts instead of Pandas Series.
