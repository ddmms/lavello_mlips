...
## 2024-06-11 - [Pandas DataFrame Iteration]
**Learning:** Iterating over large Pandas DataFrames using `df.iterrows()` is an anti-pattern that creates massive performance bottlenecks due to the creation of a new `pd.Series` object for every row.
**Action:** Always prefer converting the entire DataFrame to a list of dictionaries first using `df.to_dict('records')` before iterating. Be sure to update downstream calls that expect a Pandas Series to use the native Python dictionary instead.
