...
def parse_quadrupole(
    ...
    # We still run match, we do not want string lower allocations
...
def parse_dipole(txt: str) -> Optional[Tuple[float, float, float, float]]:
    ...
    # Fast path explicit checking using .lower() on large text string allocates
    # memory and hurts performance. Just use regex which is optimized in C.
    best = None
