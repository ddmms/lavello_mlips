...
def homo_lumo(evals, occs, thr=1e-3):
    ...
    # Find Highest Occupied Molecular Orbital (HOMO)
    h_idx = -1
    for i, o in enumerate(occs):
        if o is not None and o > thr:
            h_idx = i

    # Handle cases with no occupied orbitals
    if h_idx == -1:
        for i, o in enumerate(occs):
            if o is not None and o <= thr:
                return None, evals[i]
    ...
    # Find Lowest Unoccupied Molecular Orbital (LUMO)
    l_idx = None
    # First search above the HOMO
    for i in range(h_idx + 1, len(occs)):
        o = occs[i]
        if o is not None and o <= thr:
            l_idx = i
            break
    ...
    return evals[h_idx], (evals[l_idx] if l_idx is not None else None)
