...
def parse_charge_mult(txt: str) -> Tuple[Optional[int], Optional[int]]:
    ...
    # Fast path: Check anchored XYZ header first
    m_xyz = RE_XYZ.search(txt)
    if m_xyz:
        try:
            Q = int(m_xyz.group(1))
            M = int(m_xyz.group(2))
            return Q, M  # Early return if fast regex succeeds
        except ValueError:
            # Ignore unparsable XYZ header values; fallback to slower search
            logger.debug(
                "Failed to parse charge/multiplicity from XYZ header match %r; ignoring.",
                m_xyz.groups(),
            )

    # Slow path: Global search
