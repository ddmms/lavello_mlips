...
def parse_charge_mult(txt: str) -> Tuple[Optional[int], Optional[int]]:
    ...
    if m:
        try:
            # Fast path: if XYZ header exists, its values override everything else,
            # so we can return early and skip the expensive global regex search.
            return int(m.group(1)), int(m.group(2))
        except ValueError:
            # Ignore unparsable XYZ header values; fallback to charge/mult
            logger.debug(
