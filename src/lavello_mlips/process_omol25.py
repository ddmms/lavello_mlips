...
def homo_lumo(evals, occs, thr=1e-3):
    ...
    if occ_idx_max == -1:
        ...
        for i, o in enumerate(occs):
            if o is not None and o <= thr:
                virt_idx_min = i
                break
        return None, (evals[virt_idx_min] if virt_idx_min != -1 else None)
    ...
    lumo_idx = virt_above_min if virt_above_min != -1 else virt_min

    return evals[occ_idx_max], (evals[lumo_idx] if lumo_idx != -1 else None)


def parse_eigens(txt: str) -> Optional[Dict[str, Any]]:
    if not txt or "E(eV)" not in txt:
        return None
    ...
    while i < len(lines):
        line = lines[i]
        if "E(eV)" in line and RE_COLS.search(line):
            tag = "R"
            back = (
