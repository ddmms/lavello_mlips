...
def cog(coords):
    # ⚡ Bolt: Replaced nested Python loop with vectorized np.mean
    # Expected impact: ~2-3x speedup for 100+ atoms
    return np.mean(coords, axis=0).tolist()


def cnc(Z, coords):
    # ⚡ Bolt: Replaced nested Python loop with vectorized np.average
    # Expected impact: ~2-3x speedup for 100+ atoms
