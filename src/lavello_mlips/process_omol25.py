...
def geom_sha1(elems, coords, ndp: int = 6) -> Optional[str]:
    # ⚡ Bolt Optimization:
    # Building a single string via "".join() and encoding once with "ascii" is significantly
    # faster than iteratively calling h.update() with multiple string encodes inside a loop.
    s = "".join(
...
def homo_lumo(evals, occs, thr=1e-3):
    # ⚡ Bolt Optimization:
    # Using O(N) direct loops with early breaks avoids creating full list allocations,
    # reducing memory overhead and drastically improving execution speed.
    if not evals:
        return None, None
    n = len(occs)
    h = None
    # Find HOMO (highest index with occ > thr)
    for i in range(n - 1, -1, -1):
        if occs[i] is not None and occs[i] > thr:
            h = i
            break
    ...
    lumo_idx = None
    if h is None:
        # No occupied, so LUMO is first virtual
        for i in range(n):
            if occs[i] is not None and occs[i] <= thr:
                lumo_idx = i
                break
    else:
        # First virtual above HOMO
        for i in range(h + 1, n):
            if occs[i] is not None and occs[i] <= thr:
                lumo_idx = i
                break
        if lumo_idx is None:
            # Fallback if no virtual above HOMO
            for i in range(n):
                if occs[i] is not None and occs[i] <= thr:
                    lumo_idx = i
                    break

    return (evals[h] if h is not None else None), (
        evals[lumo_idx] if lumo_idx is not None else None
    )
