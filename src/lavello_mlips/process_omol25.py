...
def parse_charge_mult(txt: str) -> Tuple[Optional[int], Optional[int]]:
    ...
    if m:
        try:
            ...
            # Ignore unparsable XYZ header values; fallback to slow regex
            logger.debug(
                "Failed to parse charge/multiplicity from XYZ header match %r; falling back.",
                m.groups(),
            )

    Q = None
    M = None
    for m_match in RE_CHARGE_MULT.finditer(txt):
        q_match = m_match.group(1)
