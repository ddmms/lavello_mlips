...
def parse_quadrupole(
    ...
    if "buckingham" not in txt.lower():
        return None
...
def parse_charge_mult(txt: str) -> Tuple[Optional[int], Optional[int]]:
    ...
    if "charge" in txt.lower() or "multiplicity" in txt.lower():
...
def parse_eigens(txt: str) -> Optional[Dict[str, Any]]:
    ...
    if "occ" not in txt.lower() and "e(ev)" not in txt.lower():
        return None
    ...
    while i < len(lines):
        if (
            "occ" in lines[i].lower()
            and "e(ev)" in lines[i].lower()
            and RE_COLS.search(lines[i])
        ):
            tag = "R"
            back = (
