...
def parse_charge_mult(txt: str) -> Tuple[Optional[int], Optional[int]]:
    ...
    # First attempt to extract from the fast, anchored XYZ header.
    # This bypasses the slower finditer operation if a match is found.
    m = RE_XYZ.search(txt)
    if m:
        try:
            Q = int(m.group(1))
            ...
            # Ignore unparsable XYZ header values; leave Q/M as determined below.
            logger.debug(
