...
def geom_sha1(elems, coords, ndp: int = 6) -> Optional[str]:
    h = hashlib.sha1()
    # Performance optimization: Build a single string using a generator expression
    # and perform one .encode() / .update() call instead of doing it in a loop.
    # This reduces overhead for repetitive molecular fingerprinting.
    data = "".join(
