...
def parse_charge_mult(txt: str) -> Tuple[Optional[int], Optional[int]]:
    ...
    # Prioritize the fast, anchored regex (`RE_XYZ`) which checks the file header.
    # If a match is found and parsed successfully, we return early,
    # avoiding the much slower global search (`RE_CHARGE_MULT.finditer`).
    # This provides a ~20x speedup for this function on large files.
    m = RE_XYZ.search(txt)
    if m:
        try:
            Q = int(m.group(1))
            ...
            # Ignore unparsable XYZ header values, fall back to global search
            logger.debug(
