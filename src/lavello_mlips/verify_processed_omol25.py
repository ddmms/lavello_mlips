...
def main() -> None:
    ...
    # Convert DataFrame to list of dicts first to avoid the significant overhead of iterrows()
    parquet_records = df.to_dict("records")
    parquet_by_sha = {row["geom_sha1"]: row for row in parquet_records}
    parquet_by_argone_rel = {row["argonne_rel"]: row for row in parquet_records}
